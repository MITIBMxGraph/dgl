...
@torch.no_grad()
def batchwise_test(model: torch.nn.Module, num_batches: int, devit: DeviceIterator, cb: TestCallback = None):
    model.eval()

    device, = devit.devices

    results = torch.empty(num_batches, dtype=torch.short, pin_memory=True)
    total = 0
